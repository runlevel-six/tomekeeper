...
| `retain_after_read` | `interval` | How long this reader keeps what they have read before it is forgotten. Nullable, meaning "follow `TOME_RETAIN_AFTER_READ`"; zero is a real value meaning keep everything, and is distinct from nullable. Forgetting is what releases their claim, and the shared bytes go only when no claim is left — see [Retention](retention.md). |
...
| `forgotten_at` | `timestamptz` | Set when this reader's retention window let the article lapse. The row survives as a tombstone saying only "finished": expiry reads *no row* as "never seen it", so deleting it would make a reader who is done indistinguishable from a subscriber who has not got to it, and the article would never be expirable. Cleared by any act of engagement. |
