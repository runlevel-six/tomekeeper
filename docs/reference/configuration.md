...
| `TOME_USERNAME` | string | `tome` | no | Names the operator's account **when it is created**, by `tome migrate`. Changing it afterwards does nothing: a reader owns their own name and changes it under **Settings → Your name**, and `tome migrate` runs on every deploy — so honoring this later would undo their choice, which it did until v0.17.1. The Job says so when the two differ. |
| `TOME_PASSWORD` | string | — | no | Password for the **operator's account** — the one `tome migrate` seeds, not every account. Everybody else sets their own from a single-use link, and this never touches them. Read **only by `tome migrate`**, which stores an argon2id hash and derives the Fever API key from it. `tome serve` never reads it. Unset leaves any existing password alone; unset on a first run means the web interface cannot be signed into. Setting it always rotates the Fever key, so mobile clients need reconnecting. |
...
| `TOME_POLL_MIN_INTERVAL` | duration | `15m` | no | Floor for the poll interval. No feed is polled more often, **including one a reader has set a cadence for**: this is the promise made to other people's servers, so a shorter choice in the interface is raised to it. It is also where failure backoff starts. |
| `TOME_POLL_MAX_INTERVAL` | duration | `24h` | no | Ceiling for the *adaptive* interval — how rarely this will poll a feed on its own initiative. Must be at least the floor. A reader asking for less often than this is not refused, so a per-feed or general cadence may exceed it. |
...
| `TOME_FETCH_ALLOW_PRIVATE` | list | — | no | Non-public destinations the fetcher may nevertheless reach, comma-separated: CIDR blocks (`10.0.0.0/8`), bare addresses (`192.168.4.7`), and host names (`wiki.lan`), in any mixture. **Empty by default, which refuses all of them** — loopback, RFC1918, IPv6 unique-local, link-local (including the `169.254.169.254` metadata address), carrier-grade NAT, and the reserved ranges. Set this only to archive something on your own network, and name that network rather than opening every private range. A misspelled entry is a startup failure, because a setting that silently matches nothing looks exactly like a broken guard. Host names are matched exactly and are trust in whatever the name resolves to; a CIDR is checked against the address the resolver actually returned, so it holds against DNS rebinding. |
| `TOME_IMAGE_CONCURRENCY` | int | `1` | no | How many images are transcoded at once. **The setting to change if the worker is being OOM-killed.** One transcode allocates a large contiguous WASM arena — the AVIF encoder instantiates a module per call, so the cost is the call rather than the pixels, and a 3 megapixel photograph costs about as much as a 27 megapixel one. Measured high-water marks over repeated transcodes: 519MB at 1, 792MB at 2, 1100MB at 2 without the explicit release. Raise it only alongside the container's memory limit. |
| `TOME_RENDER_BROWSER_URL` | URL | — | no | CDP endpoint of a headless browser, used **only** for domains a domain rule flags `requires_js`. Empty disables rendering, which is the ordinary case: those articles stay pending rather than failing, so pointing this at a Service scaled to zero is a valid resting state. A host and port is enough — the browser's real websocket address carries a GUID that changes on every restart and is resolved at connect time. See [Enable headless rendering](../how-to/enable-headless-rendering.md). |
| `TOME_RENDER_CONCURRENCY` | int | `1` | no | How many pages may be rendered at once, capped at 4. Its own setting rather than sharing `TOME_WORKER_CONCURRENCY` for the reason `TOME_IMAGE_CONCURRENCY` has one: the cost is per page — a browser tab holding a document and its scripts — not per byte. Raise the render pod's memory limit alongside it. |
...
| `TOME_FETCH_ALLOW_PRIVATE` | testing a feed | yes | — | — | — | — | — |
