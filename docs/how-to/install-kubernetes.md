...
**From CI.** Pushing a release tag (`v1.0.1`) runs the whole pipeline and, if every
...
```bash
docker build -t ghcr.io/runlevel-six/tomekeeper:v1.0.1 .
echo "$GITHUB_TOKEN" | docker login ghcr.io -u "$GITHUB_USER" --password-stdin
docker push ghcr.io/runlevel-six/tomekeeper:v1.0.1
```
...
The manifests pin a release — `newTag: v1.0.1` in `deploy/base/kustomization.yaml`
