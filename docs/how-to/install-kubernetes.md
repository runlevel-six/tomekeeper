...
**From CI.** Pushing a release tag (`v0.17.0`) runs the whole pipeline and, if every
...
```bash
docker build -t ghcr.io/runlevel-six/tomekeeper:v0.17.0 .
echo "$GITHUB_TOKEN" | docker login ghcr.io -u "$GITHUB_USER" --password-stdin
docker push ghcr.io/runlevel-six/tomekeeper:v0.17.0
```
...
The manifests pin a release — `newTag: v0.17.0` in `deploy/base/kustomization.yaml`
